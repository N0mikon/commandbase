...
last_updated_note: "Updated after 61 commits - fixed agent paths after plugin conversion (newagents/ -> plugins/commandbase-code/agents/)"
topic: "Iterative Retrieval - Smarter Agent Context"
tags: [research, agents, retrieval, context]
status: archived
archived: 2026-02-09
archive_reason: "Complete research with only external references (everything-claude-code); 56 commits behind HEAD with no local code dependencies. Pattern documented and recommendation captured."
references:
...
- `code-locator` (plugins/commandbase-code/agents/code-locator.md): Uses broad search then refine by language/framework - one pass
- `code-analyzer` (plugins/commandbase-code/agents/code-analyzer.md): Read entry points, follow code paths - linear tracing
- `code-librarian` (plugins/commandbase-code/agents/code-librarian.md): Search, read, extract - single cycle
