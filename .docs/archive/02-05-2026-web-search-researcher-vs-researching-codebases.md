...
status: archived
archived: 02-09-2026
archive_reason: "Research complete and shipped. References pointed to old pre-plugin paths (~/.claude/agents/, ~/.claude/skills/) which no longer exist. Both components now live in plugins: commandbase-research (web-researcher) and commandbase-code (researching-code)."
references:
