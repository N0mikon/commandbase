...
last_updated_note: "Updated after 60 commits - fixed stale newskills/ paths to plugins/ structure, corrected architecture notes re plugin marketplace"
topic: "Workflow Enhancement Ideas from everything-claude-code"
tags: [research, workflow, skills, agents, hooks]
status: archived
archived: 2026-02-09
archive_reason: "Research complete. High-priority findings (learn, checkpoint) implemented as learning-from-sessions and bookmarking-code skills. All references are external to C:/code/everything-claude-code/ — no internal code dependencies. Remaining low/medium items are aspirational."
references:
...
| 🟢 Low | `/skill-create` | Add project pattern extraction | Partially covered by `creating-skills` skill (commandbase-meta plugin) |

## Implementation Status (Updated 2026-02-01)

### Completed

**`learning-from-sessions` skill** (plugins/commandbase-session/skills/learning-from-sessions/SKILL.md)
...
**`bookmarking-code` skill** (plugins/commandbase-core/skills/bookmarking-code/SKILL.md)
...
Our structure matches this for skills (plugins/<plugin>/skills/<skillname>/SKILL.md).

### Plugin vs Manual Install
everything-claude-code supports both plugin install and manual copy. Our commandbase now uses a plugin marketplace architecture (`.claude-plugin/marketplace.json`) with 8 domain plugins installed via `/plugin install`.
