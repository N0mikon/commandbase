...
last_updated_note: "Bumped git_commit after 60 commits - archived doc, added staleness note to Implementation Progress (newskills/ paths replaced by plugins/ structure, session hooks now implemented)"
...
status: archived
archived: 2026-02-09
archive_reason: "Completed external research - all references point to C:/code/everything-claude-code/ (external repo), findings consumed across multiple skills and plans, 56 commits behind HEAD with no local code dependencies"
references:
...
> **Note (2026-02-09):** The `newskills/` paths below are outdated. The codebase was restructured into a plugin marketplace architecture. Skills now live at `plugins/<plugin>/skills/<skill>/SKILL.md`. Additionally, session lifecycle hooks are now implemented in the `commandbase-session` plugin (SessionStart, Stop, PreCompact, PostToolUseFailure). This section is preserved as-is for historical context.
...
|------------------|--------|----------------|
| Security Review | Implemented | `newskills/reviewing-security/` (now `plugins/commandbase-git-workflow/skills/reviewing-security/`) |
| Verification | Partial | `newskills/validating-code/` (now `plugins/commandbase-core/skills/validating-code/`) |
| Checkpointing | Implemented | `newskills/bookmarking-code/` (now `plugins/commandbase-core/skills/bookmarking-code/`) |
| Planning | Implemented | `newskills/planning-code/` (now `plugins/commandbase-code/skills/planning-code/`) |
...
- ~~Hooks for session lifecycle~~ -- implemented in commandbase-session plugin (v2)
