# Should web-search-researcher Become a Skill or Get a Slash Command?

```yaml
git_commit: 46b48db
last_updated: 02-05-2026
topic: Architecture decision for web-search-researcher - keep as agent, convert to skill, or hybrid
...
references:
  ...
  - .docs/research/02-05-2026-web-search-researcher-vs-researching-codebases.md
```

## Research Question

Should the web-search-researcher agent be converted into a skill, have a slash command wrapper added, or take a hybrid approach? What are the pros/cons of each?

## Summary

There are three viable options, but the hybrid approach (Option C) is the strongest fit. It mirrors the proven `/researching-codebases` architecture — an orchestrator skill that spawns worker agents — while preserving web-search-researcher's role as a reusable worker. A critical discovery: **slash commands have been merged into skills** as of Claude Code 2.1.3, so "add a slash command" and "create a skill" are now the same thing.
...
### Option A: Convert web-search-researcher Agent → Skill

Replace the agent file entirely with a skill directory.

**What changes:**
- `~/.claude/agents/web-search-researcher.md` → `~/.claude/skills/researching-web/SKILL.md`
- Name transforms: `web-search-researcher` → `researching-web` (gerund form per convention)
...
**Cons:**
- **Breaks the worker pattern**: `/starting-projects` currently spawns `web-search-researcher` as a worker agent via Task tool. Skills run in main context — they can't be spawned as subagents the same way.
...
~/.claude/agents/web-search-researcher.md  (unchanged worker)
```
...
### Option C: Create an Orchestrator Skill (Parallel to /researching-codebases)

Create `/researching-web` as a full orchestrator skill that decomposes web research questions and spawns multiple web-search-researcher agents in parallel — exactly mirroring how `/researching-codebases` orchestrates codebase-* agents.
...
~/.claude/agents/web-search-researcher.md  # Unchanged worker agent
```

**Pros:**
- **Mirrors proven architecture**: Same pattern as `/researching-codebases` → codebase-* agents
...
- **Composable with /researching-codebases**: Could eventually have `/researching-codebases` spawn `/researching-web` agents when questions need both local and external context
...
| Mirrors existing patterns | No | Partially | Yes (mirrors /researching-codebases) |
...
Skill activates → Decomposes into search angles:
  1. Spawn web-search-researcher: "Next.js 15 official authentication documentation"
  2. Spawn web-search-researcher: "NextAuth vs Clerk vs Auth0 comparison 2026"
  3. Spawn web-search-researcher: "Next.js 15 auth security best practices"
...
```
- Minimum 2 web-search-researcher agents per question
...
2. **Scope overlap**: When should `/researching-codebases` delegate to `/researching-web` vs. user invoking directly?
3. **Simple queries**: Should the skill have a "fast path" that skips full orchestration for single-angle questions?
4. **Integration**: Should `/researching-codebases` gain an option to also spawn web-search-researcher agents for mixed queries?
...
- Applies the lessons from `/researching-codebases` to web research
...
- Creates a symmetric architecture: `/researching-codebases` for internal, `/researching-web` for external
