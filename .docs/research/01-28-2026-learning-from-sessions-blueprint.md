...
references:
  ...
  - newskills/learn/SKILL.md
  - newreference/pattern-learning.md
...
### 5. Existing Learn Skill vs. Blueprint

The current `learn` skill (`newskills/learn/SKILL.md`, 172 lines) is a simpler predecessor:
...
1. **Relationship to existing `learn` skill**: Will `learning-from-sessions` replace `newskills/learn/` or coexist? The blueprint targets `newskills/learning-from-sessions/` as a separate directory, but their functionality overlaps significantly.

2. **Hook implementation**: The blueprint marks the `UserPromptSubmit` hook as optional (lines 183-198). Without it, activation depends entirely on semantic matching and explicit `/learn` invocation. The forced eval hook achieves 84% activation rate -- is that desired or is the context cost (~25 lines per prompt) too high?

3. **Queue infrastructure**: The blueprint references Claude-Reflect's queue architecture conceptually but doesn't specify whether `learning-from-sessions` should implement a full queue with hooks or remain a purely on-demand skill. The "Core Workflow" section (lines 101-180) describes an on-demand workflow, not a capture-then-process flow.

4. **Reference to `creating-skills`**: Implementation requirement #12 says to "reference `creating-skills` (the active creator skill) for validation rules." No `creating-skills` skill currently exists in `newskills/` or `~/.claude/skills/`. The blueprint `.docs/plans/creating-skills-blueprint.md` may describe this planned skill.

5. **Deployment overlap**: Both the existing `learn` skill and the planned `learning-from-sessions` would deploy to `~/.claude/skills/`. If both are active, Claude's semantic matching might trigger both for similar prompts. The descriptions would need to be carefully differentiated.
