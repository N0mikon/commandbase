...
date: 2026-02-11
status: active
topic: "Old /handing-over skill preserved from commit 87a19a3 (pre-v2 deletion)"
...
# Old /handing-over Skill (Pre-v2)

Preserved from commit `87a19a3` (last version before deletion in `92113aa`).
This is the final evolved form, with session awareness and docs-writer delegation.
...
- **Session name**: [name]
- **Session ID**: [session ID from meta.json]
- **Claude UUIDs**: [list from claudeSessionIds]
...
1. If active session exists AND errors.log has entries:
