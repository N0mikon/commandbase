...
references:
  ...
  - .docs/research/01-28-2026-skill-factory-workflow.md
  - .docs/research/01-28-2026-agent-skill-creator-workflow.md
...
**How this differs from existing newskills patterns**:

The 11 existing skills in `newskills/` are all flat: one `SKILL.md` per directory, no subdirectories. The `creating-skills` skill would be the first to use `reference/` and `templates/` subdirectories. This is consistent with the progressive disclosure pattern from all three primary influences, which recommend keeping SKILL.md lean (200-300 lines) and putting depth into supporting files.
