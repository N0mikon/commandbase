...
- [ ] Consider adding to `~/.claude/CLAUDE.md`: "On MINGW, avoid heredoc for multi-line Python — write to temp file instead"
- [ ] Consider updating `/committing-changes` skill: add a guard or note about `git rm`'d files already being staged, to avoid pathspec errors during commit workflow
