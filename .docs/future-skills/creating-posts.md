...
- Should it read the project's README/CLAUDE.md for context, or rely on user description?
- Should it integrate with `/researching-web` to check what similar projects said when they launched (and avoid sounding the same)?
- Platform-specific character limits and formatting — how much should it enforce?
- Should it offer to critique a draft the user already wrote, in addition to generating from scratch?
- Tone calibration — how does the user communicate their personal voice vs just "not AI"?
